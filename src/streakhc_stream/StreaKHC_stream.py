...
from src.utils.IsoKernelOnline import IsoKernelOnline
...
def streKHC_stream(data_path, psi, t):
    ...
    train_dataset = []
    L = 5000
    iko = IsoKernelOnline(n_estimators=t, max_samples=psi)
    for i, pt in enumerate(load_data_stream(data_path), start=1):
        iko = iko.add_observation(pt[2])
        if iko.n_instance <= psi:
            ...
            if iko.n_instance == psi:
                for j, train_pt in enumerate(train_dataset, start=1):
                    l, pid, ikv = (
                        train_pt[0],
                        train_pt[1],
                        iko.transform([train_pt[2]])[0],
                    )
                    root = root.grow((l, pid, ikv), L=L, delete_node=True)
        else:
            l, pid = pt[:2]
            root = root.grow((l, pid, iko.transform([pt[2]])[0]), L=L, delete_node=True)
    return root
...
def grid_search_inode(data_path, psi, t, m, file_name, exp_dir_base):
    ...
    max_purity = 0
    for ps in psi:
        root = streKHC_stream(data_path, ps, t)
...
if __name__ == "__main__":
    main()
    # data_path = "./data/raw/spiral_inverse_sorted.csv"
    # m = 44
    # t = 200
    # psi = [3, 5, 10, 17, 21, 25]
    # file_name = "spiral"
