        ...
        if i <= train_size:
            ...
            if i == train_size:
                ...
                for j, train_pt in enumerate(train_dataset, start=1):
                    ...
                    root = root.grow((l, pid, ikv), L=window_size, delete_node=True)
        else:
            l, pid = pt[:2]
            root = root.grow(
                (l, pid, ik.transform([pt[2]])[0]), L=window_size, delete_node=True
            )
            if i >= window_size:
                if i == window_size:
...
def save_data(args, exp_dir_base):
    ...
    if not os.path.exists(file_path):
        with open(file_path, "w") as fout:
            fout.write(
                "%s\t%s\t%s\t%s\n" % ("dataset", "algorithm", "aucroc", "max_psi",)
            )
    with open(file_path, "a") as fout:
        fout.write(
            "%s\t%s\t%.2f\t%s\n"
            % (args["dataset"], args["algorithm"], args["aucroc"], args["max_psi"],)
        )
...
def save_grid_data(args, exp_dir_base):
    ...
    if not os.path.exists(file_path):
        with open(file_path, "w") as fout:
            fout.write("%s\t%s\t%s\t%s\n" % ("dataset", "algorithm", "aucroc", "psi",))
    with open(file_path, "a") as fout:
        fout.write(
            "%s\t%s\t%.2f\t%s\n"
            % (args["dataset"], args["algorithm"], args["aucroc"], args["psi"],)
        )


def grid_search_inode(
    data_path, psi, t, train_size, file_name, exp_dir_base, window_size
):
    alg = "StreaKHC"
    max_score = 0
    for ps in psi:
        root, tree_ad_score = streKHC(data_path, train_size, ps, t, window_size)
