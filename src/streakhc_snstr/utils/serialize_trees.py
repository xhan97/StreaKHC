...
def serliaze_tree_to_file_with_node_ds(root, fn):
    with open(fn, 'w') as fout:
        ...
        while not queue.empty():
            ...
            fout.write("%s\t%s\t%s\t%s\t%%.2f\t%s\n" % (curr_node_id, sibling_node_id,
                       curr_node.parent.id if curr_node.parent else "None", ns, curr_node.depth() if curr_node.parent else 0, curr_node.pts[0][0] if curr_node.is_leaf() else "None"))
