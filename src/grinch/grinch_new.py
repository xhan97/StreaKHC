...
class Grinch:
    ...
    def __init__(
        ...
        if points is not None:
            ...
            if norm == "l2":
                ...
                norms = np.linalg.norm(points, ord=np.inf, axis=1, keepdims=True)
                norms[norms == 0] = 1  # 避免除零
                self.points = points / norms
            else:
                self.points = points
            ...
            self.points = None
            self.num_points = num_points if num_points else 0
        ...
        # 用于快速查找的数据结构
        self.point_to_node = {}  # point_id -> GNode 映射
    ...
    def build_dendrogram(self):
        """构建完整的层次聚类树"""
        s = time.time()
        for i in tqdm(range(self.num_points), desc="grinch_build_dendrogram"):
            if i % 100 == 0:
                logging.info(self.stats_string())
            self.insert(i)
        logging.info("Build dendrogram completed in %s seconds", time.time() - s)

    def insert(self, point_id, point_vec=None):
        """插入一个新的数据点到聚类树中
        ...
        # 获取向量
        if point_vec is None and self.points is not None:
            point_vec = self.points[point_id]

        # 处理第一个点
        if self.point_counter == 0:
            self._create_root_node(point_id, point_vec)
            self.point_counter += 1
            return

        # 执行插入流程
        self._insert_into_tree(point_id, point_vec)
        self.point_counter += 1

        elapsed = time.time() - start_time
        logging.debug("[insert] Finished inserting point %s (%.4fs)", point_id, elapsed)

    def _create_root_node(self, point_id, point_vec):
        """创建根节点
        ...
        # 添加点信息
        label = self._get_point_label(point_id)
        self.root_node.add_pt(point_id, label=label, point_vector=point_vec)
        ...
        self.root_node.update_centroid(norm=self.norm)

        # 注册节点
        self.point_to_node[point_id] = self.root_node

        logging.debug("[insert] Created root node for point %s", point_id)

    def _get_point_label(self, point_id):
        """获取点的标签
        ...
        Returns:
            标签值或None
        """
        if self.pids is not None and point_id < len(self.pids):
            return self.pids[point_id]
        return None

    def _insert_into_tree(self, point_id, point_vec):
        """将点插入到已存在的树中
        ...
        label = self._get_point_label(point_id)
        new_point_data = (point_id, label, point_vec)
        new_leaf = sibling.split_down(new_point_data)

        # 注册新叶子节点
        self.point_to_node[point_id] = new_leaf
    ...
    def flat_clustering(self, threshold):
        ...
        # 使用GNode的flat_clustering方法
        assignments_dict = self.root_node.flat_clustering(threshold)

        # 转换为数组格式
        assignments = -1 * np.ones(self.num_points, dtype=np.int32)
        for point_id, cluster_id in assignments_dict.items():
    ...
    def write_tree(self, filename, lbls):
        ...
        with open(filename, "w") as fout:
            # 如果没有根节点，只写入结束标记
            if self.root_node is None:
                fout.write("-1\tNone\tNone\n")
                logging.info("Tree written to %s (empty tree)", filename)
                return
            ...
            while not queue.empty():
                ...
                if node.parent is None:
                    ...
                    hasattr(node.parent, "numeric_id")
                    and node.parent.numeric_id is not None
                ):
                    parent_id = node.parent.numeric_id
                else:
                ...
                if node.is_leaf():
                    ...
                    label = (
                        lbls[point_id]
                        if lbls is not None and point_id < len(lbls)
                        else "None"
                    )
                else:
                    # 内部节点
                ...
                node_id = (
                    node.numeric_id
                    if hasattr(node, "numeric_id") and node.numeric_id is not None
                    else str(node.id)
                )
                fout.write(f"{node_id}\t{parent_id}\t{label}\n")
        ...
        logging.info("Tree written to %s", filename)
