...
def get_purity(X, n_estimators, max_samples, method):
    idk = IsoKAHC(n_estimators=n_estimators,
...
def main():
    ...
    parser.add_argument('--method', '-m', type=str, required=True,
                        help='<Required> Linkage function')
    args = parser.parse_args()
    run(data_path=args.input, file_name=args.dataset, n_estimators=args.sample_size,
        max_samples_list=args.psi, method=args.method, exp_dir_base=args.outdir)
