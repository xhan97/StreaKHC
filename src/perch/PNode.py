...
class PNODE:
    ...
    def __init__(self, exact_dist_thres=30):
        self.id = "id" + "".join(
    ...
    def insert(self, pt, collapsibles=None, L=float("Inf")):
        ...
        if self.pts is not None and len(self.pts) == 0:
            ...
            if collapsibles is not None:
                heappush(
                    collapsibles, (new_leaf.parent.children_max_d, new_leaf.parent)
                )
            ...
            if collapsibles is not None and root.point_counter > L:
                ...
                if best.siblings()[0].is_leaf():
                    heappush(collapsibles, (best.parent.children_max_d, best.parent))
    ...
    def _update(self):
        ...
        if self.children:
            ...
            old_maxs = self.maxes
            self.mins = np.min(
                np.array([child.mins for child in self.children]), axis=0
            )
            self.maxes = np.max(
                np.array([child.maxes for child in self.children]), axis=0
            )
            ...
            still_have_pts = self.pts or (self.parent and self.siblings()[0].pts)
    ...
    def a_star_beam(self, pt, heuristic=lambda n, x: n.min_distance(x), beam_width=10):
            ...
            while len(best_leaves_so_far) < beam_width and frontier:
                ...
                while to_explore:
                    ...
                    if target.children:
                        ...
                        heappush(
                            best_leaves_so_far,
                            (_fast_norm_diff(target.pts[0][0], dp), target),
                        )
    ...
    def add_pt(self, pt):
        ...
        if self.pts is not None and self.point_counter > self.exact_dist_threshold:
            self.pts = None
    ...
    def recursive_rotate_if_masked(self, collapsibles=None):
        ...
        while curr_node != r and masked:
            if (
                curr_node.parent
                and curr_node.parent.parent
                and curr_node.is_closer_to_aunt()
            ):
                curr_node._rotate()
                # Collapsed mode.
                if (
                    collapsibles is not None
                    and curr_node.is_leaf()
                    and curr_node.siblings()[0].is_leaf()
                ):
                    heappush(
                        collapsibles,
                        (curr_node.parent.children_max_d, curr_node.parent),
                    )
                curr_node = curr_node.parent
    ...
    def recursive_rotate_if_unbalanced(self, collapsibles=None):
        ...
        while curr_node != r:
            ...
            rotate_order = sorted([sibling, curr_node], key=lambda x: x.point_counter)
            if (
                curr_node.parent
                and curr_node.parent.parent
                and rotate_order[0].can_rotate_for_balance()
            ):
                rotate_order[0]._rotate()
                if (
                    collapsibles is not None
                    and rotate_order[0].is_leaf()
                    and rotate_order[0].siblings()[0].is_leaf()
                ):
                    heappush(
                        collapsibles,
                        (rotate_order[0].parent.children_max_d, rotate_order[0].parent),
                    )
                curr_node = rotate_order[0].parent
            elif (
                curr_node.parent
                and curr_node.parent.parent
                and rotate_order[1].can_rotate_for_balance()
            ):
                rotate_order[1]._rotate()
                if (
                    collapsibles is not None
                    and rotate_order[1].is_leaf()
                    and rotate_order[1].siblings()[0].is_leaf()
                ):
                    heappush(
                        collapsibles,
                        (rotate_order[1].parent.children_max_d, rotate_order[1].parent),
                    )
    ...
    def valid_collapse(self):
        ...
        return (
            not self.deleted
            and self.children
            and self.children[0].is_leaf()
            and self.children[1].is_leaf()
        )
    ...
    def is_closer_to_aunt(self):
        ...
        if self.parent and self.parent.parent:
            aunt = self.aunts()[0]
            aunt_max_dist = max(
                aunt.max_distance(self.mins), aunt.max_distance(self.maxes)
            )
            other_max_dist = max(
                self.max_distance(aunt.mins), self.max_distance(aunt.maxes)
            )
    ...
    def _rotate_without_masking(self):
        ...
        if self.parent and self.parent.parent:
            ...
            aunt_min_dist = min(
                aunt.min_distance(self.mins), aunt.min_distance(self.maxes)
            )
            other_min_dist = min(
                self.min_distance(aunt.mins), self.min_distance(aunt.maxes)
            )
    ...
    def _rotate_improves_bal(self):
        ...
        if self.parent and self.parent.parent:
            ...
            bal = min(self_size, sibling_size) / max(self_size, sibling_size) + min(
                aunt_size, parent_size
            ) / max(aunt_size, parent_size)
            bal_rot = min(self_size, aunt_size) / max(self_size, aunt_size) + min(
                sibling_size, new_parent_size
